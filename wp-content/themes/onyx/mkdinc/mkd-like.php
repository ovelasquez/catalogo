...
class MikadoLike {
	...
	function add_mkd_like(){
		...
		$output = $this->like_post($post->ID);
  		...
  		$class = 'mkd-like';
  		$title = __('Like this', 'mkd');
		if( isset($_COOKIE['mkd-like_'. $post->ID]) ){
			$class = 'mkd-like liked';
			$title = __('You already liked this!', 'mkd');
		}
	...
	function add_mkd_like_portfolio_list($portfolio_project_id){
		...
		if( isset($_COOKIE['mkd-like_'. $portfolio_project_id]) ){
			$class = 'mkd-like liked';
			$title = __('You already like this!', 'mkd');
		}
...
global $mkd_like;
$mkd_like = new MikadoLike();

function mkd_like() {
	global $mkd_like;
	echo wp_kses($mkd_like->add_mkd_like(), array(
		'span' => array(
			'class' => true,
			'aria-hidden' => true,
			'style' => true,
...
function mkd_like_latest_posts() {
	global $mkd_like;
	return $mkd_like->add_mkd_like(); 
}

function mkd_like_portfolio_list($portfolio_project_id) {
	global $mkd_like;
	return $mkd_like->add_mkd_like_portfolio_list($portfolio_project_id);
}