...
add_filter( 'wp_setup_nav_menu_item', 'mkd_add_custom_nav_fields' );

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'mkd_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'mkd_edit_walker', 10, 2 );


function mkd_add_custom_nav_fields( $menu_item ) {
 ...
 * @return      void
*/
function mkd_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
 ...
 * @return      void
*/
function mkd_edit_walker($walker,$menu_id) {

	return 'Mikado_Walker_Nav_Menu_Edit_Custom';
...
if (!class_exists('mkd_type1_walker_nav_menu')) {
	class mkd_type1_walker_nav_menu extends Walker_Nav_Menu {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($mkd_options['page_transitions'] == "0")):
			...
			$wide_background = '';
			if(isset($mkd_options['enable_wide_manu_background']) && $mkd_options['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && isset($mkd_options['enable_dropdown_menu_item_icon']) && $mkd_options['enable_dropdown_menu_item_icon'] == "yes" ){
...
if (!class_exists('mkd_type2_walker_nav_menu')) {
	class mkd_type2_walker_nav_menu extends Walker_Nav_Menu {
...
if (!class_exists('mkd_type3_walker_nav_menu')) {
    class mkd_type3_walker_nav_menu extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            // depth dependent classes
            if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($mkd_options_theme16['page_transitions'] == "0")):
                $active = 'active';
            endif;
...
if (!class_exists('mkd_type4_walker_nav_menu')) {
    class mkd_type4_walker_nav_menu extends Walker_Nav_Menu {
