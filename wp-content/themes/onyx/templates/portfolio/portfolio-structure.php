...
if(get_post_meta($id, "mkd_page_background_color", true) != ""){
	$container_styles .= 'background-color: '. esc_attr(get_post_meta($id, "mkd_page_background_color", true)).';';
}

//get current portfolio template
$portfolio_template = 'small-images';
if(get_post_meta($id, "mkd_choose-portfolio-single-view", true) != "") {
	$portfolio_template = get_post_meta($id, "mkd_choose-portfolio-single-view", true);
} elseif($mkd_options['portfolio_style'] !== '') {
	$portfolio_template = $mkd_options['portfolio_style'];
}

if(get_post_meta($id, "mkd_content-top-padding", true) != ""){
	$content_style = 'padding-top: '.get_post_meta($id, "mkd_content-top-padding", true).'px';
}else{
	$content_style = "";
}
?>

<div class="container" <?php mkd_inline_style($container_styles); ?>>
