<?php
	...
	class WC_Prdctfltr {
		...
		function prdctfltr_scripts() {
			...
			if ( is_rtl() ) {
				wp_register_style( 'prdctfltr-main-rtl', self::$url_path .'lib/css/prdctfltr-rtl.css', false, self::$version );
				wp_enqueue_style( 'prdctfltr-main-rtl' );
			}
			...
			if ( !isset( $prdctfltr_global['init'] ) ) {
				...
				$curr_args = array(
					...
					'localization' => array(
						...
						'noproducts' => __( 'Ningún producto encontrado!', 'prdctfltr' ),
						'clearall' => __( 'Limpiar todos los filtros', 'prdctfltr' ),
...
<?php
					...
					switch( $k ) {
						...
						case 's' :
							if ( isset( $pf_activated['s'] ) && isset( $active_filters['s'] ) ) {
								echo __( 'Búsqueda', 'prdctfltr' ) . ': '. $pf_activated['s'];
							}
						break;
						case 'products_per_page' :
							if ( isset( $pf_activated['products_per_page'] ) ) {
								echo $pf_activated['products_per_page'] . ' ' . __( 'Productos por pagina', 'prdctfltr' );
							}
						break;
						case 'sale_products' :
		...
		public static function get_top_bar_showing() {
			...
			if ( self::$settings['instance']['wc_settings_prdctfltr_noproducts'] !== '' && self::$settings['instance']['total'] == 0 ) {
				echo ' / ' . __( 'Ningún producto encontrado!', 'prdctfltr' );
			}
				...
				if ( $pf_step_filter !== '' ) {
					...
					echo ' / ' . __( 'Mostrando el único resultado', 'prdctfltr' );
				}
			}
			else if ( self::$settings['instance']['total'] <= self::$settings['instance']['per_page'] || -1 == self::$settings['instance']['per_page'] ) {
				if ( $pf_step_filter !== '' ) {
					echo ' / ' . __( 'Mostrando', 'prdctfltr') . ' ' . self::$settings['instance']['total'] . ' ' . __( 'resultados', 'prdctfltr' );
				}
				else {
					echo ' / ' . __( 'Mostrando', 'prdctfltr') . ' ' . self::$settings['instance']['total'] . ' ' . __( 'resultados', 'prdctfltr' );
				}
				...
				if ( $pf_step_filter !== '' ) {
					...
					echo ' / ' . __( 'Mostrando', 'prdctfltr' ) . ' ' . self::$settings['instance']['first'] . ' - ' . self::$settings['instance']['last'] . ' ' . __( 'de', 'prdctfltr' ) . ' ' . self::$settings['instance']['total'] . ' ' . __( 'resultados', 'prdctfltr' );
				}
			}
